...
> Kit note (added at packaging): this doc was written for the first tool (Strategic Use Cases). It now ships as one of **three** static tools behind a landing hub — (1) Strategic Use Cases `use-cases.html`, (2) Live Conversation Guide `live-guide.html`, (3) Use Case Profile Builder `profile-builder.html` — served from `site/` with `index.html` as the hub. All three share the same architecture class described below: single-file, static, stateless, dependency-free, native-print PDF export. The deploy topology (§8) is now "serve `site/` on one Railway service"; the doc's per-tool detail (§4) still describes tool 01 specifically. A fuller per-tool doc can follow if needed.
